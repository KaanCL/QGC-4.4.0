...
Item {
    ...
    property var    _mainRtspUrl : "rtsp://192.168.144.108:"
    property var    _videoRtspUrl : _mainRtspUrl + "554/stream=1"
    property  var   _termalRtspUrl: _mainRtspUrl + "555/stream=2"
    ...
    function changeRtspUrl(){
        ...
        if(videoStream){
            ...
            if(currentRtspUrl == _videoRtspUrl){
                _videoSettings.rtspUrl.rawValue = _termalRtspUrl
                _videoManager._videoSourceChanged()
            }
            else if(currentRtspUrl == _termalRtspUrl){
                  _videoSettings.rtspUrl.rawValue = _videoRtspUrl
             _videoManager._videoSourceChanged()}
            else{
             _videoSettings.rtspUrl.rawValue = _videoRtspUrl
              _videoManager._videoSourceChanged()
            ...
            console.log( _videoSettings.videoSource.rawValue)
    ...
    Rectangle{
      ...
      anchors.left:  _pipOverlay.left
      anchors.right: _pipOverlay.right
      visible : true//_pipOverlay.visible
      ...
      Text{
         ...
         color:"white"
         font.pointSize: Screen.width / 90
      }


    MouseArea{
      anchors.fill: parent
      onClicked: changeRtspUrl()
    }
